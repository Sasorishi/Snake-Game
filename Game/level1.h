...
{		
	...
	SDL_Surface* pomme3 = IMG_Load("sprites/apple.png");
	...
	while(continuer)
	{
		...
		float newTime = SDL_GetTicks() / 150; //La valeur pour la update le corps du serpent
		...
		if (collision(bodybox[0], pomme3POS))
			{
				random3 = rand() % 650;
				pomme3POS.x = random3;
				pomme3POS.y = random3;
		...
		SDL_BlitSurface(pomme3, NULL, ecran, &pomme3POS);
		SDL_UpdateWindowSurface(fenetre);
	}
