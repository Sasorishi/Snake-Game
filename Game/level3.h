...
{		
	...
	//Chargement d'image
	SDL_Surface* mur = IMG_Load("sprites/wall.png");
	SDL_Surface* mur2 = IMG_Load("sprites/wall.png");
	SDL_Surface* mur3 = IMG_Load("sprites/wall.png");
	SDL_Surface* mur4 = IMG_Load("sprites/wall.png");	
	...
	while(continuer)
	{
		...
		float newTime = SDL_GetTicks() / 65; //La valeur pour la update le corps du serpent
		...
		if (up)
		{
			...
			//Si il y a collision
			if ((collision(bodybox[0], murPOS)) || (collision(bodybox[0], mur2POS)) || (collision(bodybox[0], mur3POS)) || (collision(bodybox[0], mur4POS))) 
			{
				//Vitesse annuler
				bodybox[0].y += speed;
		...
		else if (down) 
		{
			bodybox[0].y += speed * delta;
			//Si il y a collision
			if ((collision(bodybox[0], mur2POS)) || (collision(bodybox[0], mur3POS)) || (collision(bodybox[0], mur4POS)))
			{
				//Vitesse annuler
				bodybox[0].y -= speed;
				Mix_PlayMusic(dead, 0);
		...
		//Nouvelle position quand pomme et mur
		if (collision(pommePOS, murPOS) || collision(pommePOS, mur2POS) || collision(pommePOS, mur3POS) || collision(pommePOS, mur4POS))
		{
			random = rand() % 650;
			pommePOS.x = random;
			pommePOS.y = random;
		...
		SDL_BlitSurface(mur2, NULL, ecran, &mur2POS);
		SDL_BlitSurface(mur3, NULL, ecran, &mur3POS);
		SDL_BlitSurface(mur4, NULL, ecran, &mur4POS);
		SDL_UpdateWindowSurface(fenetre);
	}
